...
for brand, data in car_data.items():
    ...
    if year >= min_year and engine >= min_engine and price <= max_price:
        car_for_user.append((brand, price, year, engine, car_type, color))

car_for_user .sort(key=lambda x: x[1])

top_5 = car_for_user [:5]

for i, (brand, price, year, engine, ctype, color) in enumerate(top_5, 1):
    print(f"{i:2}. {brand:12}  {year} р.  {engine:4.1f} л  {price:6}$  {color} {ctype}")
if not top_5:
    print("Жоден автомобіль не відповідає критеріям")